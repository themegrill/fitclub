...
class FitClub_Welcome_Notice {
	...
	public function welcome_notice() {
		if ( ! get_option( 'fitclub_admin_notice_welcome' ) ) {
	...
	public function welcome_notice_markup() {
		$dismiss_url = wp_nonce_url(
			...
			'_fitclub_notice_nonce'
		);
		?>
		<div id="message" class="notice notice-success fitclub-notice">
			<a class="fitclub-message-close notice-dismiss" href="<?php echo esc_url( $dismiss_url ); ?>"></a>

			<div class="fitclub-message__content">
				<div class="fitclub-message__image">
					<img class="fitclub-screenshot" src="<?php echo esc_url( get_template_directory_uri() ); ?>/screenshot.jpg" alt="<?php esc_attr_e( 'fitclub', 'fitclub' ); ?>"/><?php // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped, Squiz.PHP.EmbeddedPhp.SpacingBeforeClose ?>
				</div>
	...
	public function hide_notices() {
		if ( isset( $_GET['fitclub-hide-notice'] ) && isset( $_GET['_fitclub_notice_nonce'] ) ) { // WPCS: input var ok.
			if ( ! wp_verify_nonce( wp_unslash( $_GET['_fitclub_notice_nonce'] ), 'fitclub_hide_notices_nonce' ) ) { // phpcs:ignore WordPress.VIP.ValidatedSanitizedInput.InputNotSanitized
			...
			$hide_notice = sanitize_text_field( wp_unslash( $_GET['fitclub-hide-notice'] ) );

			// Hide.
			if ( 'welcome' === $_GET['fitclub-hide-notice'] ) {
				update_option( 'fitclub_admin_notice_' . $hide_notice, 1 );
			} else { // Show.
				delete_option( 'fitclub_admin_notice_' . $hide_notice );
			}
		}
	}
	...
	public function welcome_notice_import_handler() {
		...
		if ( 'activated' === $state ) {
			$response['redirect'] = admin_url( '/themes.php?page=demo-importer&browse=all&fitclub-hide-notice=welcome' );
